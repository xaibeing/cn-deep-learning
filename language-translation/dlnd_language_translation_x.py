...
t=set(source_text.split())
t=set(target_text.split())
len(t)
...
#t.append('<EOS>')
#
#t=(sentences[0]+str(' <EOS>')).split()
#t=(sentences[0]+' <EOS>').split()
#
#t=sentences[0].split().append('<EOS>')
#t=sentences[0].split().extend(['<EOS>'])
#t=['<EOS>'].extend(sentences[0].split())

#sentences = source_text.split('\n')
#
#source_vocab_to_int, source_int_to_vocab = helper.create_lookup_tables(source_text)
##print(word for sentence in sentences for word in sentence)
##t = [source_vocab_to_int[word] for sentence in sentences for word in sentence ]
#t = [[source_vocab_to_int[word] for word in sentence.split()] for sentence in sentences]
##t = [sentence for sentence in sentences]
#len(t)
#len(t[0])
#len(t[10])
...
# source_int_text 是一些句子的list，比如[[2, 54, 23 ...], [43, 3, 55...], ......]
...
# Batch Size
#batch_size = 128
#max_sentence = max([len(sentence) for sentence in source_int_text])
...
# In[ ]:
# 在解码阶段RNN的输入序列。首先输入<GO>，然后是期望的输出word的序列，最后一个<EOS>不需要输入了。
...
def process_decoding_input(target_data, target_vocab_to_int, batch_size):
    ...
    """
    # TODO: Implement Function
#    target_data_size = target_data.get_shape().as_list()
#    target_data_size = target_data.shape.as_list()
#    print('target_data_size', target_data_size)
#    if(target_data_size[0] is None):
#        target_data_size = tf.shape(target_data)
#        print('1')
#    print('target_data_size', target_data_size)
#    print('target_data.shape', target_data.shape)
#    print('target_data.shape[0]', target_data.shape[0])
#    print('target_data.shape.as_list()', target_data.shape.as_list())
#    target_data_size = 3
    
#    go_array = np.full((target_data_size[0],1), target_vocab_to_int['<GO>'])
#    go_array = tf.fill((target_data_size[0],1), target_vocab_to_int['<GO>'])
...
#    print('decoding_scope', decoding_scope.name)
#    print('dec_embed_input.shape', dec_embed_input.shape)
...
#        print('dec_embed_input.shape', dec_embed_input.shape)
#        print('dec_embed_output.shape', dec_embed_output.shape)
...
#    print('input_shape', input_data.shape.as_list())
#    a = tf.ones([input_shape[0], sequence_length])
#    train_logits, inference_logits = 1,2
    train_logits, inference_logits = seq2seq_model(
        tf.reverse(input_data, [-1]), targets, keep_prob, batch_size, sequence_length, len(source_vocab_to_int), len(target_vocab_to_int),
        encoding_embedding_size, decoding_embedding_size, rnn_size, num_layers, target_vocab_to_int)
#    print('seq2seq_model() done')
...
# 留了batch_size个句子做validation，其它句子都用作training
...
#            print('target_batch.shape[1]', target_batch.shape[1])
